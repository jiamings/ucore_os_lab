...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2012011271 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    if(blkoff = offset % SFS_BLKSIZE) {
        ...
        if(sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) goto out;
        if(sfs_buf_op(sfs, buf, size, ino, blkoff)) goto out;
        alen += size;
        if(!nblks--)  goto out;
        buf += size;
        blkno ++;
    ...
    while(nblks--) {
        if(sfs_bmap_load_nolock(sfs, sin, blkno++, &ino)) goto out;
        if(sfs_block_op(sfs, buf, ino, 1))  goto out;
        buf += SFS_BLKSIZE;
        alen += SFS_BLKSIZE;
    }
    if(size = endpos % SFS_BLKSIZE) {
        if(sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) goto out;
        if(sfs_buf_op(sfs, buf, size, ino, 0))   goto out;
        alen += size;
    }
