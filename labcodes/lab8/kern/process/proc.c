...
alloc_proc(void) {
     ...
     */
     //LAB5 2012011271 : (update LAB4 steps)
    /*
	 ...
	 */
     ...
     //LAB6 2012011271 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2012011271 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2012011271
    //LAB8:EXERCISE2 2012011271  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    if(copy_files(clone_flags, proc))   goto bad_fork_cleanup_kstack;   // swap errors
    if(copy_mm(clone_flags, proc))  goto bad_fork_cleanup_fs;
	...
	//LAB5 2012011284 : (update LAB4 steps)
   /* Some Functions
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2012011271  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
     ...
     */
     ...
     struct elfhdr helf;
     struct elfhdr *elf = &helf;
     struct proghdr hprog;
     struct proghdr *ph = &hprog;
     //(3.2) get the entry of the program section headers of the bianry program (ELF format)
     if(load_icode_read(fd, elf, sizeof(struct elfhdr), 0)) goto bad_elf_cleanup_pgdir;
     ...
     int phno;
     for (phno = 0; phno < elf->e_phnum; phno ++) {
         
         if(load_icode_read(
             fd, 
             ph, 
             sizeof(struct proghdr), 
             elf->e_phoff + sizeof(struct proghdr) * phno)
         ) 
     ...
     //(3.5) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
      ...
      //(3.6.1) copy TEXT/DATA section of bianry program
         while (start < end) {
             ...
             if(load_icode_read(fd, page2kva(page) + off, size, from))
                goto bad_cleanup_mmap;
