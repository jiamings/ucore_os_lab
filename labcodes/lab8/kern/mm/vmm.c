...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) {
    ...
    */
    /*LAB3 EXERCISE 1: 2012011271*/
    pde_t * pdep = mm->pgdir;
    ptep = get_pte(pdep, addr, 1);              //(1) try to find a pte, if pte's PT(Page Table) isn't existed, then create a PT.
    if (*ptep == 0) {
        pgdir_alloc_page(pdep, addr, perm);     //(2) if the phy addr isn't exist, then alloc a page & map the phy addr with logical addr
    }
    else {
    /*LAB3 EXERCISE 2: 2012011271
